...
SEDB_del %>%
  mutate(Meas_method_group = case_when(Meas_method %in% c("Runoff plot") ~ "Runoff plot",
    Meas_method %in% c("Erosion pins") ~ "Topography",
    Meas_method %in% c("Caesium 137") ~ "Caesium 137",
    Meas_method %in% c("Be7", "Pb210") ~ "Be7",
    Meas_method %in% c("Radioisotopic", "Tracer") ~ "Radioisotopic",
    ...
    Meas_method %in% c("Limnigraph", "Microprofiler", "Profilometer", "Topography") ~ "Topography",
    TRUE ~ "OTH"  )) -> SEDB_del
...
bind_rows(
  ...
  SEDB_del %>% 
    ...
    count(Study_midyear) %>% 
    mutate(Year = floor(Study_midyear), Num_study = n, Source = "(b) Number of measurements by measure year compiled in SoilErosionDB")) %>% 
...
bind_rows(
  SEDB_del %>% 
    ...
    dplyr::select(Longitude, Latitude, ER_annual, ER_g_m2_h) %>% 
...
SEDB_del %>% dplyr::select(ER_g_m2_h, Runoff_mm_h, IGBP.y) %>% na.omit() %>% 
  ggplot(aes(x = Runoff_mm_h, y = ER_g_m2_h, col = IGBP.y)) +
  geom_point() +
  geom_smooth(method = "lm") +
...
SEDB_del %>% 
  ...
  summarise(ER_mean = mean(ER_annual),
            obs = n(),
            se = sd(ER_annual) / sqrt(obs)) -> SEDB_IGBP
...
SEDB_del %>% 
  ...
  summarise(ER_mean = mean(ER_annual),
            obs = n(),
            se = sd(ER_annual) / sqrt(obs)) %>% 
  filter(Soil_Group != "SC") -> SEDB_soil
...
SEDB_del %>% 
  ...
  summarise(ER_mean = mean(ER_annual),
            obs = n(),
            se = sd(ER_annual) / sqrt(obs)) -> SEDB_meas

SEDB_meas$Meas_method_group <- c("BAT", "Be7", "Cs137", "CAT", "Field" , "FRS",
                                 "Model", "OTH", "IST", "Plot" , "TOP")
...
# Field scale
SEDB_del %>% 
  dplyr::select(Field_scale, ER_annual) %>% 
  filter(!is.na(ER_annual)) %>% 
  group_by(Field_scale) %>% 
  summarise(ER_mean = mean(ER_annual),
            obs = n(),
            se = sd(ER_annual) / sqrt(obs)) -> SEDB_scale
SEDB_scale$Field_scale <- c("OTH", "Catchment", "Plot", "Slope")
...
(plot_igbp / plot_soil) | (plot_meas / plot_scale) -> plot_catg
plot_catg + plot_annotation(tag_levels = 'a', tag_prefix = '(',  tag_suffix = ')')

# ggsave("outputs/Figure 3. Sistribution of erosion and runoff.png", width = 10, height = 8, dpi = 300, units = "in" )
```
...
lm_er_var <- function(sdata, var_column) {
  ...
  outputs = tibble(var_num = var_column,
                   R2 = sum_lm$r.squared,
                   n = nrow(sdata_agg),
                   p = sum_lm$coefficients[2,4] %>% round(5) )
...
```{r plot ER_annual vs num variables}
...
plot_er_var(SEDB_del, Pannual_del) +
  ...
  annotate("text", x = 3500, y = -8, label = "italic(R) ^ 2 == 0.07", hjust = 0, parse = TRUE) +
  ggtitle('(a)') -> plot_precp


# b. vs annual temperature
plot_er_var(SEDB_del, Tannual_del) +
  labs(x = expression(Annual~temperature~"("~degree~C~")"),
      y = expression('Erosion,'~log~"(t ha"^{-1}~"yr"^{-1}~")")) +
  annotate("text", x = 15, y = -10, label = "italic(R) ^ 2 == 0.13", hjust = 0, parse = TRUE) +
  ggtitle('(b)') -> plot_temp
...
plot_er_var(SEDB_del, Plant_cover_C) +
  labs(x = expression(Plant~coverage~"(%)"),
      y = expression('Erosion,'~log~"(t ha"^{-1}~"yr"^{-1}~")")) +
  annotate("text", x = 25, y = -3, label = "italic(R) ^ 2 == 0.25", hjust = 0, parse = TRUE) +
  ggtitle('(c)') -> plot_plant

# d. vs plant clay
plot_er_var(SEDB_del, Soil_clay) +
  labs(x = expression(Soil~clay~"(%)"),
      y = expression('Erosion,'~log~"(t ha"^{-1}~"yr"^{-1}~")")) +
  annotate("text", x = 40, y = -4, label = "italic(R) ^ 2 == 0.11", hjust = 0, parse = TRUE) +
  ggtitle('(d)') -> plot_clay
...
plot_er_var(SEDB_del, Soil_BD) +
  labs(x = expression(Soil~BD~"("~g~m^3~")"),
      y = expression('Erosion,'~log~"(t ha"^{-1}~"yr"^{-1}~")")) +
  theme(axis.title.y = element_blank()) +
  annotate("text", x = 1.5, y = -3, label = "italic(R) ^ 2 == 0.22", hjust = 0, parse = TRUE) +
  ggtitle('(e)') -> plot_bd
...
plot_er_var(SEDB_del, Soil_pH) +
  labs(x = expression(Soil~pH),
      y = expression('Erosion,'~log~"(t ha"^{-1}~"yr"^{-1}~")")) +
  theme(axis.title.y = element_blank()) +
  annotate("text", x = 7.5, y = -6, label = "italic(R) ^ 2 == 0.18", hjust = 0, parse = TRUE) +
  ggtitle('(f)') -> plot_pH

# g vs SOC
plot_er_var(SEDB_del, Soil_C_.) +
  labs(x = expression(Soil~SOC~"(%)"),
      y = expression('Erosion,'~log~"(t ha"^{-1}~"yr"^{-1}~")")) +
  theme(axis.title.y = element_blank()) +
  annotate("text", x = 8, y = -6, label = "italic(R) ^ 2 == 0.07", hjust = 0, parse = TRUE) +
  ggtitle('(g)') -> plot_soc
...
SEDB_agg_fieldarea %>% 
  ...
  theme(axis.title.y = element_blank()) +
  scale_size_continuous(name = "Obs (n)") +
  annotate("text", x = 10, y = -10, label = "italic(R) ^ 2 == 0.24", hjust = 0, parse = TRUE) +
  ggtitle('(h)') -> plot_area

(plot_precp / plot_temp / plot_plant / plot_clay ) | (plot_bd / plot_pH / plot_soc / plot_area) 
# ggsave("outputs/Figure 4. Erosion v relationship.png", width = 9, height = 10, dpi = 300, units = "in" )
```
...
```{r}
(plot_mpet / plot_sand / plot_n) | (plot_cn / plot_ksat / plot_l) 
ggsave("outputs/Figure SX. Erosion var.png", width = 9, height = 8, dpi = 300, units = "in" )
```
